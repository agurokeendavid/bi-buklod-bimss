...
import type { MyProfile, ReferenceDataItem, SubmitMemberUpdateRequestResult, UpdateMemberRequest } from "@/lib/types/member";
...
export default function MemberUpdateRequestPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const [profileResponse, civilStatusesResponse, suffixesResponse, officeUnitsResponse] = await Promise.all([
        fetchWithAuth("/api/my/profile"),
      ...
      if (!profileResponse.ok || !civilStatusesResponse.ok || !suffixesResponse.ok || !officeUnitsResponse.ok) {
        setLoadError("Failed to load your profile.");
        return;
      }

      const profile = (await profileResponse.json()) as MyProfile;
      ...
      setSuffixId(profile.suffixId ?? NO_SUFFIX);
      setDateOfBirth(profile.dateOfBirth);
      setPlaceOfBirth(profile.placeOfBirth);
      setCivilStatusId(profile.civilStatusId);
      setJoiningReason(profile.joiningReason ?? "");
      setPositionDesignation(profile.positionDesignation);
      setOfficeUnitId(profile.officeUnitId);
      setPermanentAppointmentDate(profile.permanentAppointmentDate ?? "");
      setIsLoaded(true);
    }
  ...
  function handleCancel() {
    if (isDirty && !window.confirm("Discard this request? Your changes will be lost.")) {
      return;
    }
    router.push("/my");
  }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setSubmitError(`Failed to submit your request (${response.status}).`);
        return;
      }

      (await response.json()) as SubmitMemberUpdateRequestResult;
      setIsDirty(false);
      toast.success("Your update request was submitted for review.");
      router.push("/my");
    } finally {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[980px] flex-col">
      <Card className="rounded-xl shadow-none">
        <CardHeader>
          <CardTitle className="text-[14.5px] font-semibold">Request a profile change</CardTitle>
          <CardDescription>
            Changes go to a Membership Officer for review before they take effect. Fields marked
            <RequiredMark /> are required. Contact details are not covered here — those update directly.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {notLinked ? (
